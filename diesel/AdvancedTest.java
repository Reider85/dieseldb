...
public class AdvancedTest {
    ...
    public void runTests() {
        try {
            ...
            try {
                ...
                throw new RuntimeException("insertWithDuplicateSequencePrimaryKey succeeded unexpectedly, test failed");
            } catch (RuntimeException e) {
                if (e.getCause() instanceof IllegalArgumentException) {
                    LOGGER.log(Level.INFO, "Expected failure in insertWithDuplicateSequencePrimaryKey: {0}", e.getCause().getMessage());
                } else {
    ...
    private void insertRecords() {
        ...
        List<String> columns = Arrays.asList("USER_CODE", "NAME", "AGE", "BALANCE"); // ID is generated by SEQUENCE
    ...
    private void insertWithDuplicateSequencePrimaryKey() {
        ...
        LOGGER.log(Level.INFO, "Executing INSERT with duplicate SEQUENCE primary key: {0}", query);
        long startTime = System.nanoTime();
        database.executeQuery(query, null);
        long endTime = System.nanoTime();
        double durationMs = (endTime - startTime) / 1_000_000.0;
        LOGGER.log(Level.WARNING, "Insert with duplicate SEQUENCE primary key succeeded unexpectedly in {0} ms", String.format("%.3f", durationMs));
    }
    ...
    private void insertWithDuplicateUniqueIndex() {
        String query = "INSERT INTO USERS (ID, USER_CODE, NAME, AGE, BALANCE) VALUES (1003, 'CODE1002', 'User1002Duplicate', 25, 1500.00)";
