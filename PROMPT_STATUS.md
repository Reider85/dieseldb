...
| - | IN_PROGRESS (2026-09-05) | MEDIUM | diesel/format/, Table.java, StorageFormat.java, Database.java | Storage-format framework refactoring (analytics/refactoring.md): phases 1-5 done - diesel.format package (TableFormat/FormatRegistry/TableData/CsvFormat/ParquetFormat), Table saveToFile delegates to CsvFormat, TableData-driven ParquetWriter/ParquetReader core + FormatSupport bridge, per-table storage.format resolution (storage.format.<TABLE> -> default -> global), config default back to CSV. Phase 6 (final integration/tests/docs) remaining. Quick gate 42/0/0/2, heavy gate 42/0/0 BUILD SUCCESS |
