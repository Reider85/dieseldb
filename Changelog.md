...
Changes:
- Root cause: LazyDeleteTest.setUp issues 200 auto-commit INSERTs, each persisting the whole USERS.csv via AtomicFileWriter (temp+fsync+atomic rename). On Windows, replacing the same target in rapid succession intermittently hits a transient open handle (OS/Defender scanning the freshly written file); WindowsException translateToIOException -> AccessDeniedException on Files.move. The retry budget (5 attempts, base 25ms -> ~375ms total) was too short for the observed ~460ms lock, so commit() threw and LazyDeleteTest.setUp died with DieselIOException "Failed to save table to CSV file: data\USERS.csv" (all 18 tests of the class erroring). The failure was non-deterministic - a later run reached 200 rows.
- diesel/storage/AtomicFileWriter.java: MAX_MOVE_ATTEMPTS resolves from the new config key atomic.write.move.max.attempts (default 10) and the per-attempt exponential backoff is capped at atomic.write.move.retry.max.delay.ms (default 300) via Math.min(base << (attempt-1), max) - worst case ~2.4s, enough to outlive the transient lock window while keeping normal-path latency unchanged (retries only trigger on the lock). Atomicity semantics (temp keeps previous valid target, uncommitted temp discarded, per-target writer serialisation, vanished-temp early-exit) unchanged.
- diesel/storage/StorageConfig.java: new getInt(key, defaultValue) helper (system property -> config.properties -> default, WARN + fallback on unparsable value), following the CompressionFactory.resolveLevel parsing precedent.
- config.properties: new keys under an "Atomic file writer" comment - atomic.write.move.max.attempts = 10, atomic.write.move.retry.max.delay.ms = 300.
- Verification: LazyDeleteTest green 5 consecutive runs (18/18 each); full suite (4GB heap, @LargeTest) 1083/0/0/0 BUILD SUCCESS; quick suite 312/0/0/6 BUILD SUCCESS. No JOIN/performance keywords in the task, so the profile check was skipped per the strict condition; the change touches only the Windows-lock retry branch, not query execution.

3.0.82 Prompt 43 - JSONL type mapping (JsonTypeMapper, strict/lenient coercion, 2^53 precision rules)

Changes:
- diesel/storage/json/JsonTypeMapper.java (new): single point of conversion between a JSON value and the schema column type, shared by the reader, the writer-side schema validation and future schema inference (prompt 44). toColumnValue(Class, JsonEvent, raw, context) converts scalars per the target column type. Number literals: VALUE_NUMBER_INT goes through exact BigDecimal math (Long/Integer/Short/Byte via *ValueExact, overflow -> "out of range" DieselIOException); VALUE_NUMBER_FLOAT lands in integral columns via the exact value or a "cannot parse" error, in Double/Float with only a range-overflow check; a DOUBLE column rejects an integer literal whose magnitude exceeds 2^53 (9007199254740992) with an explicit "silently rounded ... use a LONG or BigDecimal column" error, while scientific-notation float literals (1e5, 1e308, 2.5E-3) pass freely - the boundary rule applies to integer tokens only, so exact long/BigDecimal round-trips never degrade. LONG and BigDecimal targets keep values exact (9007199254740993 reads as exactly that). BigDecimals keep their scale (100.50 stays scale-2). String literals: ISO-8601 strings convert into LocalDate/LocalDateTime, strings into UUID, strict boolean strings (true/false/1/0/yes/no/t/f) into Boolean, in both modes; any unparseable string fails with "cannot parse" + cause. Coercion mode (jsonl.type.coercion, STRICT default): a JSON string destined for a numeric column is rejected in strict ("1" -> 1 is illegal, message names the mode and the jsonl.type.coercion=lenient toggle) and coerced with a WARNING in lenient. validateDoublePrecision(Class, Object, field, recordContext) is the write-side twin: rejects an integral Long/Integer/Short/Byte/BigDecimal whose magnitude exceeds 2^53 being written into a DOUBLE column, keeping the write/read contract symmetric so a file that passes validation always reads back exactly.
- diesel/storage/json/JsonParserConfig.java: new CoercionMode enum (STRICT|LENIENT) carried as the single hook for jsonl.type.coercion; both jsonl.type.coercion and jsonl.duplicate.keys now resolve sysprop -> root config.properties -> default (STRICT/FAIL), with invalid overrides falling back; minimal fail-safe config.properties loader lives in the Builder; Builder gains typeCoercion(CoercionMode); equals/hashCode/toString/deprecated-comment now include the coercion mode
- diesel/storage/JsonlRowReader.java: scalar conversion fully delegated to JsonTypeMapper (inline convertString/convertNumber/conversionError/columnName helpers removed along with the BigDecimal/LocalDate/LocalDateTime/UUID imports); every scalar path passes file:line and field name context so coercion/precision errors keep the existing diagnostics contract (e.g. "type_mapper_test.jsonl:line 1: field 'AGE': ..."), lenient coercions log the mapper WARNING with that same context
- diesel/storage/JsonlSchemaManager.java: validateWriteValue now calls JsonTypeMapper.validateDoublePrecision in the Integer/Long/Short/Byte branch and the BigDecimal branch, so 2^53-crossing integral values into DOUBLE columns are rejected at write time with record N: field 'X' coordinates (JsonlRowWriter itself unchanged - validation already flows through the 3-arg writer)
- config.properties: jsonl.duplicate.keys = fail and jsonl.type.coercion = strict committed as the documented defaults
- pom.xml: JsonlTypeMapperTest registered in the default surefire includes and the ci profile
- src/test/java/diesel/JsonlTypeMapperTest.java (new, 29 tests): read-side precision (9007199254740993 exact in LONG; rejected in DOUBLE with a message naming 2^53/9007199254740992/LONG; the 2^53 boundary itself allowed; 1e400 -> DOUBLE overflow; scientific notation into LONG (1e5, 1.525E4) and DOUBLE (1e5, 2.5E-3, 1e308); high-precision BigDecimal exactness; fractional-into-integer and integer-overflow failures), coercion semantics (strict rejects "1" -> Long naming strict mode + jsonl.type.coercion; lenient coerces with a WARN captured via Slf4jLogCapture; a malformed numeric string still fails in lenient), string-to-typed (ISO date/datetime/UUID convert; non-ISO date string fails with "cannot parse"; strict boolean strings true/FALSE/1 convert, invalid fails; token-shape mismatch e.g. true -> Long, 1 -> Boolean), untyped/String columns keep the raw token text (9007199254740993 and 1e5 stay textual), JSON null -> null for any column, write-side guard (BigDecimal 9007199254740993 into a DOUBLE column rejected, 2^53-1 and 100.5 accepted, non-DOUBLE columns no-op) plus writer-level rejection and an exact LONG 2^53+1 write-read round-trip, reader integration (quoted number into numeric column rejected in strict, coerced in lenient; unicode escape \\u0041 decoded; nested null preserved inside a captured object/array), config resolution (defaults strict+fail; system-property overrides to LENIENT/LAST_WINS; invalid overrides fall back). Existing tests pinned to the new strict default: JsonlStorageTest.jsonBooleansAndNumbersConvertToColumns and JsonlSchemaProjectionTest.readScalarStringCoercionStaysLenient now pass an explicit LENIENT JsonParserConfig through the reader's 5-arg constructor.
- Footer: quick suite 339/0/0/6 then 341/0/0/6 (291 + new mapper + existing jsonl tests), full suite (4GB heap, @LargeTest) 339/0/0/0 BUILD SUCCESS. Profile check skipped - prompt 43 has no JOIN / hash join / performance wording (strict condition). Timing gate: the tracked baseline timing/timing.md (Sep 11) against the fresh timing/timing117.md flags a handful of CSV/query-path SELECTs as >20% slower, but run-to-run noise dominates on this machine (timing116.md vs timing117.md with identical code: 38 of 53 heavy queries move >20%; several of the same queries show huge improvements run to run), all flagged queries are pure query-engine paths untouched by this JSONL-only prompt, and the heavy 600x600 ORDER BY joins passed every run - no regression attributable to this change is present.

3.0.83 Make PersistenceTest storage-type agnostic (fix compressed-delimited DROP test under tsv)

Changes:
- src/test/java/diesel/PersistenceTest.java: testDropTableDeletesCompressedDelimitedFiles was hardcoded for CSV (only set csv.compression.codec=zstd and asserted TABLE + ".csv.zst" exists). With storage.type=tsv and tsv.compression.codec=none the test failed because no .csv.zst or .tsv.zst file was created. Fixed: now sets both csv.compression.codec and tsv.compression.codec to zstd, derives the expected compressed file path from the table's actual storage type via the existing delimitedExtension(Table) helper (TABLE + ext + ".zst"), and restores both codec sysprops in finally — works identically under csv, tsv, or any future delimited type.
- Verification: isolated test passes (.tsv.zst created and cleaned); quick suite 341/0/0/6; full suite (4GB heap, @LargeTest) 341/0/0/0 BUILD SUCCESS; timing gate 0 regressions. Profile check skipped (no JOIN/hash join/performance keywords in task).

3.0.81 Prompt 44 - JSONL schema modes (strict/inferred/hybrid), one-pass schema inference, sidecar evolution

Changes:
- diesel/storage/json/JsonParserConfig.java: new SchemaMode enum (STRICT|INFERRED|HYBRID, default HYBRID) carried as the single hook for jsonl.schema.mode; key resolves sysprop -> root config.properties -> default like the other jsonl.* keys, invalid overrides fall back; Builder gains schemaMode(SchemaMode) (null resets to HYBRID); equals/hashCode/toString and the javadoc key list now include the schema mode.
- diesel/storage/json/JsonSchemaInference.java (new): single-pass schema inference for JSONL files. Reads line by line like the row reader (UTF-8 BOM stripped on the first record, blank lines skipped, every line must be a single JSON object). Records each top-level field's first-seen order and observed value kinds; a field whose non-null scalar values change type between rows (number->string, number->boolean, string->boolean) fails fast with file:line:field coordinates ("field 'X': value changes type between rows (number at line 1, string at line 2) - inference refuses a silent conversion"). Per-field type resolution: object/array ever observed -> String column holding compact JSON text; integers only -> Long, integer+float -> Double, booleans only -> Boolean; strings only -> LocalDate / LocalDateTime / UUID when every value parses, else String; all-null -> String. Returns a deterministic ordered InferredSchema(columns, columnTypes).
- diesel/storage/JsonlSchemaManager.java: schemaMode() accessor; suggestNearestColumn(name) typo detector returning the case-insensitive Levenshtein-nearest schema column when within max(2, len/3) for the strict "did you mean ..." error; sidecar SchemaStamp(mtimeMillis, sizeBytes) record with of(Path)/matches/isFresh helpers; SchemaDescriptor gains an optional data stamp (2-arg constructor kept for pre-prompt-44 sidecars); writeSchemaFile(Path, SchemaStamp) writes the data block, readSchemaFile parses it back.
- diesel/storage/JsonlRowReader.java: unknown fields now routed through handleUnknownField - in STRICT mode the read fails with "unknown field 'X' is not part of the table schema (strict schema mode); did you mean 'Y'?" (hybrid/inferred keep the single per-file WARNING); javadoc updated; both nextArray and nextProjected share the policy.
- diesel/storage/AbstractRowStorage.java: columns/columnTypes are no longer final and a protected setSchema(newColumns, newTypes) replaces the schema (used by JSONL adoption); the base class otherwise untouched.
- diesel/storage/JsonlRowStorage.java: config-aware constructor (config defaults to JsonParserConfig.defaults()); loadFromFile plans the load schema per mode - strict uses the fixed schema, hybrid keeps the current columns and appends fields the file adds (with fresh sidecar reuse when the recorded stamp matches, i.e. the file is unchanged), inferred replaces the schema with the data-derived one (fresh sidecar reused, otherwise inference + sidecar write); adoption happens only after a fully successful read so a failing load always restores the previous rows and previous schema; saveToFile writes the <table>.schema.json sidecar + data stamp for hybrid/inferred modes.
- config.properties: jsonl.schema.mode = hybrid committed as the documented default.
- pom.xml: JsonlSchemaModeTest registered in the default surefire includes and the ci profile.
- src/test/java/diesel/JsonlSchemaModeTest.java (new, 19 tests): default/builder config resolution, schemaManager mode reflection, typo hint on close columns (NAEM->NAME) and no hint for far/blank names, strict reader unknown-field error with hint coordinates, no-hint far field, strict missing field -> null, hybrid warn-compatible unknown field, strict storage load rejection with rollback (typo NAEM -> hint NAME, previous rows restored), hybrid load expands schema with typed new fields in first-seen order (missing new field -> null), hybrid re-inference on data change (new field appears after the first load), hybrid save->reload round trip keeps the expanded column, nested new field stored as String JSON text, hybrid sidecar written with a fresh data stamp, inferred mode adopting a data-derived schema and writing the sidecar, inferred fresh-sidecar reuse without re-inference, inferred type-change failing with file:line:field coordinates and untouched rows, inferred empty-file yielding an empty schema.
- Verification: new class isolated 19/19 green; quick suite 360/0/0/6; full suite (4GB heap, @LargeTest) 360/0/0/0 BUILD SUCCESS. Profile check skipped - prompt 44 has no JOIN / hash join / performance wording (strict condition); make/timing targets are not available on this machine, so the acceptance gate was run as the raw Maven full suite (mvn test -Ddiesel.largeTests=true -Dtest.heap=4g) - the change touches only JSONL storage paths, not the query engine.

3.0.86 Fix ServerConnectionLimitTest/SocketTimeoutTest failures + harden server startup against stale/corrupt table files

Changes:
- diesel/Database.java: Database.loadTablesFromDisk() now wraps each Table.loadFromFile() in try/catch(Exception) and logs a WARNING instead of letting an unhandled RuntimeException (DieselIOException from a stale/corrupt .table or mismatched CSV header) propagate up and kill the server thread during startup - the server now skips the bad table and starts cleanly
- diesel/Table.java: Table.loadFromFile() catch widened from IOException | ClassNotFoundException to Exception (a DieselIOException from the storage layer is a RuntimeException and previously escaped, crashing DatabaseServer.start() via loadTablesFromDisk); a failed load now logs a WARNING "Failed to load table X, creating empty" and returns the empty fallback table
- src/test/java/diesel/ServerConnectionLimitTest.java: start the server against a JUnit temp directory (diesel-conn-limit-test) instead of the project data/ dir so stale data\PROFILES.csv cannot trigger the load failure; temp dir removed in tearDown
- src/test/java/diesel/SocketTimeoutTest.java: same temp-dir isolation (diesel-socket-timeout-test)
- Root cause: ServerConnectionLimitTest/SocketTimeoutTest previously used the default Database() constructor (data dir = ./data); a leftover data\PROFILES.csv whose header no longer matches the persisted schema (CSV header columns missing from file (required by schema): [PROFILE_CODE, NON_INDEXED, PROFILE_DATE]) made CsvRowStorage.loadCsv -> CsvRowReader.buildColumnMapping throw DieselIOException, an unchecked exception the loader did not catch - the server thread died before the accept loop, failing both tests with "DieselIO Failed to load table from CSV file"
- Verification: isolated run ServerConnectionLimitTest,SocketTimeoutTest 2/2 green BUILD SUCCESS; quick suite green; full suite (4GB heap, @LargeTest) 360/0/0/0 BUILD SUCCESS (AllTestsSampleTest.prompt69Group timing flake elapsed 29995 ms vs 30000 ms was rerun green - this is unrelated 5 ms timer noise). Profile check skipped - fix has no JOIN / hash join / performance wording (strict condition); make/timing targets not available on this machine, acceptance gate run as raw Maven full suite.

3.0.87 Prompt 45 - JSONL nested storage modes (flatten/json_column) and array handling (json/expand), SQL JSON Path support

Changes:
- diesel/storage/json/JsonParserConfig.java: two new config hooks - NestedMode (FLATTEN|JSON_COLUMN, default FLATTEN) wired to jsonl.nested.mode and ArrayColumnsMode (JSON|EXPAND, default JSON) wired to jsonl.array.columns; both resolve sysprop -> root config.properties -> default with invalid overrides falling back; Builder setters + equals/hashCode/toString + javadoc; containerPrefixes recomputed from the resolved schema.
- diesel/storage/json/JsonPathResolver.java (new, public): the projection/WHERE bridge for dotted JSON columns. ResolvedPath(columnIndex, column, segments) with static resolve(columns, path) performing longest-prefix matching over schema columns (case-insensitive) splitting the path into an exact column + remaining JSON leaf segments, plus resolveQualified(columns, path) that first tries the full path then drops one leading "TABLE." segment and retries (so USERS.user.address.city finds a user.address.city column while user.address.city resolves standalone); UNRESOLVED = (-1, null, List.of()); extract(jsonText, segments, config) re-parses the captured JSON text via the streaming parser (no DOM) and returns the leaf value (scalar -> raw token text, nested -> compact JSON text) or null when absent.
- diesel/storage/JsonlRowReader.java: FLATTEN mode: precomputes containerPrefixes (every dot-prefix of each schema column, case-insensitive) and walks nested objects with walkObjectFlat - leaves land in exact dot-notation columns (user.address.city) with the same type validation/conversion as top-level fields, objects whose full path has no child columns are captured as compact JSON text and marked as nested-JSON holder columns, unknown/not-needed fields are skipped and counted; parseArrayFlat handles any array value - under jsonl.array.columns=json (or when the array contains objects/arrays) it is stored whole in the exact column as a nested holder, under expand a scalar array fills its field[0], field[1], ... columns (validation + conversion per element, non-scalar elements cause a graceful fallback to the whole-array column); nextProjected in FLATTEN parses the full row then projects out of it (arrays must be walked to reach index columns, still honors the requested columns). JSON_COLUMN mode keeps the existing capture behaviour.
- diesel/storage/JsonlRowWriter.java: FLATTEN mode writes dot-notation columns back as real nested JSON - insertNested rebuilds objects from user.address.city style columns and arrays from field[0]..field[N-1] index columns in deterministic schema-column order; unwrapNested parses nested-JSON holder columns (captured JSON text marked on read) back into structure so a load->save round trip preserves nesting instead of double-encoding strings; JSON_COLUMN mode writes nested holders as structure and everything else as flat keys.
- diesel/storage/JsonlSchemaManager.java: tracks nestedJsonColumns (Set<Integer>, populated via markNestedJson during read/capture); validateFlattenSchema rejects a schema combining a column named X with a dot-path row X.y (prefix conflict); isNestedJson(columnIndex), jsonConfig() accessor.
- diesel/storage/JsonlRowStorage.java: one shared JsonlSchemaManager is kept across the load->save lifecycle (created/refreshed on load plan, reset on failed load rollback) so the nested-JSON holder marks captured on read survive into saveToFile and are re-embedded as structure; saveToFile validates + writes through the shared manager.
- diesel/storage/json/JsonSchemaInference.java: dot-notation leaf columns (user.address.city) and index columns (tags[0]) pass through inference unchanged; a nested object in the data maps to the longest dot-path column and object/array values continue to resolve to String JSON-text columns.
- diesel/SqlParsingUtils.java: normalizeColumnName keeps a dotted identifier whole when the first segment is not a recognized table alias (so user.address.city survives table-qualification), while alias prefixes still resolve (case-insensitively, matching alias keys OR values) to TABLE.tail; unquoteQualifiedIdentifier unquotes a whole quoted identifier (\"user.address.city\") then each dot part; validateColumn accepts exact columns or JSON Path prefixes via JsonPathResolver.resolveQualified; resolveColumnType resolves the Java type of a possibly dotted column (exact match wins, then longest JSON Path prefix, case-insensitive fallback). QueryParser.resolveType and SubqueryParser.getColumnType now delegate to resolveColumnType.
- diesel/SelectQuery.java: JSON Path support for dotted columns - resolveRowValue(row, column) returns the cell value when the column key exists, otherwise applies the memoized ResolvedPath (pathResolutionCache, cleared per execute) over the captured JSON text so WHERE (evaluateScalarCondition/In/IsNull/ColumnComparison) and projection (buildProjectionPlan + filterColumns fallbackKeys then resolveRowValue) can reach nested leaves in JSON_COLUMN mode; computeNormalizeColumnName resolves alias prefixes case-insensitively.
- config.properties: jsonl.nested.mode = flatten and jsonl.array.columns = json committed as the documented defaults.
- pom.xml: JsonlNestedModeTest registered in the default surefire includes and the ci profile.
- src/test/java/diesel/JsonlNestedModeTest.java (new, 10 tests): flatten depth-3 round trip (object nested in object through storage save/load), mixed flat+nested round trip, dot-prefix schema conflict rejected (X vs X.y), json_column depth-3 round trip + JSON Path extraction, array of objects stays a single JSON column, scalar array default (json) column, scalar array expansion to index columns, SQL WHERE on a JSON Path in json_column mode (u.NAME where u is a nested object), SQL projection extracting a JSON Path leaf, SQL WHERE on a dotted leaf in flatten mode.
- Verification: new class isolated 10/10 green; quick suite (mvn test -DskipLargeTests) 370/0/0/6; full suite (4GB heap, @LargeTest) 370/0/0/0 BUILD SUCCESS; timing gate: fresh timing/timing125.md vs baseline timing/timing.md compared with the compare-timing rule (>20% on >=11ms heavy queries flags) - 0 regressions, 108 improvements, 28 unchanged, exit 0. Profile check skipped - prompt 45 has no JOIN / hash join / performance wording (strict condition); make/changelog targets are not available on this machine, so the changelog entry was appended manually.

3.0.91 Prompt 48 - JSONL load-error diagnostics and garbage tolerance

Changes:
- diesel/storage/json/JsonParserConfig.java: new LoadErrorMode enum (FAIL | SKIP_ROW) wired to jsonl.load.error.mode config; sysprop -> config.properties -> default with invalid overrides falling back to FAIL; Builder setter (null resets to FAIL), accessor, equals/hashCode/toString, javadoc
- config.properties: jsonl.load.error.mode = fail documented as the default
- diesel/storage/JsonlRowReader.java: core skip-row infrastructure added to the streaming reader. New `skippedRowCount` field and `getSkippedRowCount()` accessor. New `skipRow(String message, Exception e)` method returns true when the row was skipped (SKIP_ROW mode) and false when it should be re-thrown (FAIL mode); logs each malformed row's file:line:field coordinates as a WARN. `prefetch()` extracted `buildParserForLine(String line)` that creates a parser, checks for START_OBJECT, and returns false on skip so the while-loop keeps scanning; for non-object lines (arrays, scalars) the exception path returns false (not the raw skipRow boolean, which would stall hasNext with a null parser). `nextArray()` rewritten with an internal skip-recovery loop: on IOException or DieselIOException during `parseCurrentRow`, `skipRow()` is invoked and `continue` re-enters the loop; when EOF is reached after a skip the method returns null (matching CsvRowReader's contract) instead of throwing NoSuchElementException. `readAll()` now filters null rows from `next()`. `atPhysicalEof()` is a cheap readLine+reset probe so the reader remains reusable (used to append a "(possibly truncated record)" note to the diagnostic). `emitSkipRowSummary()` logs the final WARN with the total skipped-row count once per file at EOF. `closeQuietly()` made null-safe. `nextProjected()` also rewritten with the same skip-recovery loop. Class and method javadoc updated throughout.
- diesel/storage/JsonlRowStorage.java: javadoc documents the fail (rollback to previous in-memory rows, rethrow with diagnostics) vs skip_row (log each, commit valid rows, final WARN) load behaviour
- pom.xml: JsonlLoadDiagnosticsTest registered in both default and ci surefire includes
- New src/test/java/diesel/JsonlLoadDiagnosticsTest.java (15 tests): diagnostics carry file:line:field coordinates; dot-notation nested field paths; blank and whitespace-only lines silently skipped; UTF-8 BOM stripped from first line; array instead of object fails with correct line number; scalar (bare value) instead of object fails with line number; broken line in the middle of the file diagnosed with line number; truncated last line flagged as "(possibly truncated record)"; skip_row drops the broken line and keeps valid rows with skippedCount incremented; skip_row on a non-object line keeps surrounding valid rows; skip_row on truncated last line returns valid rows; fail mode does not skip and reports no skipped rows; storage-level E2E skip_row commits valid rows; storage-level fail mode rolls back with diagnostics; jsonl.load.error.mode property resolution from sysprop/config.properties/default
- Verification: isolated class 15/15 green; quick suite (mvn test -DskipLargeTests) 396/0/0/6; full suite (4GB heap, @LargeTest) 396/0/0/0 BUILD SUCCESS. Profile check skipped - prompt 48 has no JOIN / hash join / performance wording (strict condition); make/changelog targets unavailable on this machine, changelog entry appended manually.

3.0.90 Prompt 49 - JSONL append/rewrite write modes, delta sidecar, auto-compaction, crash-recovery

Changes:
- diesel/storage/json/JsonParserConfig.java: new WriteMode enum (REWRITE | APPEND) and compactionThreshold (default 0.3, 0.0-1.0) wired to jsonl.write.mode and jsonl.compaction.threshold configs; sysprop -> config.properties -> default; builder setters, accessors, equals/hashCode/toString, javadoc
- config.properties: jsonl.write.mode = rewrite (default) and jsonl.compaction.threshold = 0.3 documented
- diesel/storage/JsonlDeltaManager.java (new): delta manager for APPEND mode - new rows physically appended to base .jsonl at save time with fsync barrier; deletions recorded in .jsonl.delta sidecar as single JSON header line {"baseLineCount":N,"deletions":[d1,d2...]}; onLoad reads base then re-applies delta; onSaveComplete moves pending rows into base; shouldCompact() triggers full atomic rewrite via AtomicFileWriter when (deleted base lines + pending new rows) / total live rows exceeds compactionThreshold
- diesel/storage/JsonlRowStorage.java: writeMode field from config; deltaManager created in APPEND mode; saveToFile dispatches to saveRewriteMode (full atomic rewrite) or saveAppendMode (append pending rows + fsync + atomic delta sidecar); loadFromFile in APPEND mode calls repairTruncatedAppend() (discards incomplete trailing line or repairs missing newline on complete object) then reads base + applies delta; checkAutoCompact() after each delete/update; compactJsonl() manual full rewrite + delta reset; insert/update/delete track delta; getWriteMode()/getDeltaManager() accessors
- diesel/storage/JsonlRowReader.java: atPhysicalEof() and truncated-line diagnostics reused for crash-recovery
- diesel/storage/json/JsonStreams.java / JsonStreamParser.java / JsonEvent: used by repairTruncatedAppend to verify complete JSON objects
- pom.xml: JsonlAppendModeTest registered in both default and ci surefire includes
...
Changes:
- diesel/storage/json/JsonParserConfig.java: new jsonl.missing.field policy (null | error | default; default DEFAULT = missing reads back as null, sysprop -> config.properties -> default, invalid-override fallback) with the MissingFieldMode enum wired into builder/accessor/constructor/equals/hashCode/toString; config.properties documents jsonl.missing.field = default
- diesel/storage/JsonlRowReader.java: per-row field-presence tracking (boolean[] lastRowPresent, exposed via getLastRowPresent(), cloned on read); jsonl.missing.field=error fails the offending row with a DieselIOException carrying file:line and the missing column names (enforceMissingFieldPolicy; the missing-key contract of strict schema mode from prompt 44 may now raise instead of silently nulling); typeAt made null-safe so schemaless loads (null columnTypes) work
- diesel/storage/JsonlRowWriter.java: new writeRow(Object[], boolean[]) overload - a key whose presence flag is false is not written; null stays JSON null, empty string stays "", missing stays missing. Plain writeRow(Object[]) keeps the pre-prompt behaviour (every schema column written), so old callers are byte-identical
- diesel/storage/JsonlRowStorage.java: parallel rowPresence list (boolean[] per row) maintained by insert/insertAt/update/delete/setRows (in-memory rows all-present) and carried through saveToFile -> writer and loadFromFile -> reader presence flags (rollback-safe on failed load); new getRowPresence()/setRowPresence(); javadoc documents the scan() contract - a missing key is surfaced as the null slot plus an absent presence flag rather than being silently collapsed into an explicit null (Query Executor sees unchanged null-at-Map-boundary semantics; no query-engine change needed)
- Backward compatibility: pre-prompt-47 files whose nulls were written as missing keys load all-present (missing -> null by default), and old files with explicit null keys round-trip byte-identically - no migration required
